...
class StorePublisher(Publisher):
    ...
    def notify(self):
        print("Publisher:", 'blue'), "Оповещаю подписчиков..."
        subscribers_reacts = []
    ...
    def goods_arrival(self, goods):
        ...
        print("Publisher:", 'blue'), f"Поступил новый товар - {self.new_goods}"
        self.notify()
...
def client_code():
    ...
    first_sneakers_subscriber = SneakersSubscriber("Елена")
    print(store.attach(first_sneakers_subscriber))
    second_sneakers_subscriber = SneakersSubscriber("Виктор")
    print(store.attach(second_sneakers_subscriber))
    first_hoodies_subscriber = HoodiesSubscriber("Дмитрий")
